...
static const char* GetPhysicalPinName(long pin_type)
{
	...
	default:                              return "Unknown Crossbar Pin Type¡ªPlease report!";
	}
}
...
static HRESULT setup_crossbar_options(IAMCrossbar *cross_bar, enum dshowDeviceType devtype, AVFormatContext *avctx)
{
	...
	fprintf(stderr, "dshow_crossbar: Crossbar Switching Information for %s:\n", device_name);
	hr = IAMCrossbar_get_PinCounts(cross_bar, &count_output_pins, &count_input_pins);
	if (hr != S_OK) {
		fprintf(stderr, "dshow_crossbar: Unable to get crossbar pin counts\n");
		return hr;
	}
	...
	for (i = 0; i < count_output_pins; i++) {
		...
		if (pin_type == PhysConn_Video_VideoDecoder) {
			...
			if (video_input_pin != -1) {
				fprintf(stderr, "dshow_crossbar: Routing video input from pin %d\n", video_input_pin);
				hr = IAMCrossbar_Route(cross_bar, i, video_input_pin);
				if (hr != S_OK) {
					fprintf(stderr, "dshow_crossbar: Unable to route video input from pin %d\n", video_input_pin);
					return AVERROR(EIO);
				}
			}
		} else if (pin_type == PhysConn_Audio_AudioDecoder) {
			if (audio_input_pin != -1) {
				fprintf(stderr, "dshow_crossbar: Routing audio input from pin %d\n", audio_input_pin);
				hr = IAMCrossbar_Route(cross_bar, i, audio_input_pin);
				if (hr != S_OK) {
					fprintf(stderr, "dshow_crossbar: Unable to route audio input from pin %d\n", audio_input_pin);
					return hr;
				}
			}
		} else {
			fprintf(stderr, "dshow_crossbar: Unexpected output pin type, please report the type if you want to use this (%s)", GetPhysicalPinName(pin_type));
		}

		hr = IAMCrossbar_get_IsRoutedTo(cross_bar, i, &route_to_pin);
		if (hr != S_OK) {
			fprintf(stderr, "dshow_crossbar: Unable to get crossbar is routed to from pin %d\n", i);
			return hr;
		}
		fprintf(stderr, "dshow_crossbar: Crossbar Output pin %d: \"%s\" related output pin: %ld ", i, GetPhysicalPinName(pin_type), related_pin);
		fprintf(stderr, "dshow_crossbar: current input pin: %ld ", route_to_pin);
		fprintf(stderr, "dshow_crossbar: compatible input pins: ");

		for (j = 0; j < count_input_pins; j++)
		{
			hr = IAMCrossbar_CanRoute(cross_bar, i, j);
			if (hr == S_OK)
				fprintf(stderr, "dshow_crossbar: %d ", j);
		}
		fprintf(stderr, "dshow_crossbar: \n");
	}
	...
	for (i = 0; i < count_input_pins; i++)
	{
		...
		if (hr != S_OK) {
			fprintf(stderr, "dshow_crossbar: unable to get crossbar info audio input from pin %d\n", i);
			return hr;
		}
		fprintf(stderr, "dshow_crossbar: Crossbar Input pin %d - \"%s\" ", i, GetPhysicalPinName(pin_type));
		fprintf(stderr, "dshow_crossbar: related input pin: %ld\n", related_pin);
	}
	return S_OK;
}
	...
	IBaseFilter *device_filter, enum dshowDeviceType devtype, AVFormatContext *avctx)
{
	...
	if (devtype == VideoDevice && ctx->show_analog_tv_tuner_dialog) {
		...
		if (hr == S_OK) {
			...
			fprintf(stderr, "dshow_crossbar: unable to find a tv tuner to display dialog for!");
		}
	}
	if (devtype == AudioDevice && ctx->show_analog_tv_tuner_audio_dialog) {
		...
		if (hr == S_OK) {
			...
			fprintf(stderr, "dshow_crossbar: unable to find a tv audio tuner to display dialog for!");
		}
	}
